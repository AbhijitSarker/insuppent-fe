...
const EmailModal = ({ open, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 rounded-3xl">
            ...
            {/* Modal panel */}
            <div className="rounded-3xl absolute left-1/2 bottom-0 w-full -translate-x-1/2 h-[90vh] bg-white flex flex-col border-t border-border animate-slideUpModal">
                ...
                {/* Lead info */}
                <div className="p-6 pb-2 mx-8 border-b border-border">
                    <div>

                    <div className="font-semibold text-lg mb-2">Email copy for <span className="text-primary">{sampleLead.name}</span></div>
                    <div className="flex gap-2 text-xs text-muted-foreground flex-wrap ">
                        <span className="flex items-center gap-1 bg-muted py-1 rounded"><MaterialIcon icon="email" size={16} /> {sampleLead.email}</span>
                        <span className="flex items-center gap-1 bg-muted py-1 rounded"><MaterialIcon icon="home_work" size={16} /> {sampleLead.address}</span>
                        <span className="flex items-center gap-1 bg-muted py-1 rounded"><MaterialIcon icon="location_on" size={16} /> {sampleLead.state}</span>
                        <span className="flex items-center gap-1 bg-muted py-1 rounded"><MaterialIcon icon="home" size={16} /> {sampleLead.type}</span>
                    </div>
                    </div>
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {sampleEmails.map((email, idx) => (
                                <div key={idx} className="relative flex flex-col items-stretch">
                                    ...
                                    <div className='w-min'>
                                        <div
                                            className="px-4 py-1 bg-white border border-gray-300 font-semibold text-sm shadow-sm"
                                            style={{
                                                borderBottom: 'none',
                                    ...
                                    <div
                                        className="border rounded-tl-none border-gray-300 rounded-xl bg-white p-4 flex flex-col min-h-[350px] relative"
                                        style={{ boxShadow: '0 2px 8px 0 rgba(60,72,88,0.06)' }}
                                    >
                                        {/* Subject row */}
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="font-bold text-base leading-tight">Subject: {email.subject}</div>
                                            <Button
                                                size="icon"
                                                variant="ghost"
                                                title="Copy subject"
                                            ...
                                            {copiedIdx === `subject-${idx}` && (
                                                <span className="text-xs text-success bg-muted px-2 py-1 rounded shadow ml-2">Copied!</span>
                                            )}
                                        </div>
                                        {/* Email body */}
                                        <pre className="whitespace-pre-wrap text-sm flex-1 mb-2" style={{ fontFamily: 'inherit' }}>{email.body}</pre>
